...
e) We renamed the attributes that used to use "_" with no symbol because some of our attributes turned out to be keywords in SQL, so we renamed them to avoid keywords. Also we renamed "Host" to "Hosts" because "Host" is also a keyword.
